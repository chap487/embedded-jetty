...
public class BlackScholes {
	...
	public static void main(String... args) throws Exception {
		...
		double price = calculate(true, 100, 105, 0.01, .25, .15);
		
		System.out.print("call price of 105 strike with spot at 100: " + price);
	}
	/**
	 * 
	 * @param callOption
	...
	public static double calculate(boolean callOption, double s, double k, double r, double t, double v) {
		...
		//System.out.println("    ----- ");
		//System.out.println(" in BlackScholes:calculate(" + callOption + "," + s + "," + k + "," + r + "," + t + "," + v);
		...
		System.out.print("in BlackScholes.callOption() result-[" + blackScholesOptionPrice + "] ");
	...
	public static double calculate(OptionDetails req) {
		...
		double response = calculate(req.callOption, req.spotPriceOfUnderlying,
	...
	public static OptionDetails calculateWithGreeks(OptionDetails req) {
		...
		double price = calculate(req.callOption, req.spotPriceOfUnderlying,
				req.strikePrice, req.riskFreeInterestRate, req.timeToExpire,
				req.volatility);
		
		OptionGreeks greeks = BlackScholesGreeks.calculate(req);
		
		req.optionValue = price;
		req.delta = greeks.delta;
		req.gamma = greeks.gamma;
		req.rho = greeks.rho;
		req.theta = greeks.theta;
		req.vega = greeks.vega;
	...
	public static double cumulativeDistribution(double x) {
		...
		//System.out.println(" in BlackScholes:cumulativeDitibution(" + x + ")");
	...
	public static double standardNormalDistribution(double x) {
		...
		//System.out.println(" in BlackScholes:standardNormalDistribution(" + x + ")");
